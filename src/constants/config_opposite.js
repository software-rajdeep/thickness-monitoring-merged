 ...
 * For opposite mode, only sensors A and B are returned.
 * Sensor C (if present in sensor_network.json) is used only for side-by-side mode.
 * @param {string} mode - "sbs" (side-by-side) or "opposite"
 * @returns {Promise<object>} The sensor configs from the server
 */
export async function fetchSensorConfigs(mode = "opposite") {
  try {
    const base = SERVER;
